function [A, At, G, W, Gw,scale] = op_p_nufft(p, N, Nn, No, Ns, param)
...
if param.gen_only_fft_op
    [A, At, ~, scale] = op_nufft([0, 0], N, Nn, No, Ns);
    G = [];
    W = [];
    Gw = [];
else
    if ~param.use_nufft_blocks
        ...
        [A, At, Gw, scale] = op_nufft(cell2mat(p), N, Nn, No, Ns);
        ...
        for q = 1:R
            ...
            [~, ~, Gb, scale] = op_nufft([p{q, 1} p{q, 2}], N, Nn, No, Ns);
