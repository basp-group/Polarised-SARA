...
try
    if gen_figures
        ...
        for i = 1:num_tests
            ...
            if exist('Gw', 'var')
                residual = At(Gw' * (cell2mat(ay0) - Gw * ys));
            end
            if ~exist('Gw', 'var') && exist('G', 'var')
                residual = zeros(size(W{1}));
                for q = 1:R
                    residual(W{q}) = residual(W{q}) + G{q}' * (ay0{q} - G{q} * ys(W{q}));
                end
            ...
            if ~gen_only_average_figures
                ...
                try
                    ...
                    subplot(2,2,1); hold on; imagesc(log10(max(imn, 1e-4))); colorbar; axis image; title('Original image');
                    subplot(2,2,2); hold on; imagesc(log10(max(soln_, 1e-4))); colorbar, axis image; title('Recovered image');
                    subplot(2,2,3); hold on; imagesc(log10(max(dirtyn_, 1e-4))); colorbar, axis image; title('Dirty image');
        ...
        if gen_only_average_figures
            ...
            subplot(2,2,1); hold on; imagesc(log10(max(imn, 1e-4))); colorbar; axis image; title('Original image');
            subplot(2,2,2); hold on; imagesc(log10(max(soln, 1e-4))), colorbar, axis image; title('Recovered image');
            subplot(2,2,3); hold on; imagesc(log10(max(dirtyn, 1e-4))), colorbar, axis image; title('Dirty image');
