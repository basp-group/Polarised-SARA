...
for t = 1:param_algo.tmax
    ...
    for i = 1:P
        ...
        if proj_l2 == 1
            dual_tilde3{i} = dual_var3{i} + sigma3*(Gw*A(2*St_im{i}-St_im_old{i}));
            b1 = sc((dual_tilde3{i}./sigma3)-y{1,n_test}{i,1},epsilon{n_test})+y{1,n_test}{i,1}; % Projecting onto l2 ball
            dual_var3{i} = dual_tilde3{i} - sigma3*b1;
            g3{i} = At(Gw'*(dual_var3{i}));
        else
            g3{i} = At(Gw'*((Gw*A(St_im{i})-y{1,n_test}{i,1})));
        end
